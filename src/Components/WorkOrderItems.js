...
export default function WorkOrderItems(props) {
  ...
  for (let photo in props.photos) {
    props.photos[photo] &&
      parsedPhotos.push(
        <img key={photo} src={props.photos[photo].path} alt="user" />
      );
  }
  ...
  const MobileComponent = () => (
    <div className="card workorder-item">
      <div className="workorder-content-mobile status-and-title">
        ...
        <div className="">
          {props.time_completed ? (
            <span className="tag is-success">Finished</span>
          ) : props.time_started ? (
            <span className="tag is-info">Ongoing</span>
          ) : (
      ...
      <div className="workorder-content-mobile status-and-title">
        <div className="workorder-content-mobile">
          ...
          {props.created_on && <p>{props.created_on.slice(0, 10)}</p>}
        </div>
        <div>
          {isAuthenticated &&
            user.email !== 'hafizthesakora@gmail.com' &&
            !props.time_completed &&
            props.time_started && <FinishButton id={props._id} />}
          {isAuthenticated &&
            user.email !== 'hafizthesakora@gmail.com' &&
            !props.time_completed &&
            !props.time_started && <StartButton id={props._id} />}
        </div>
      </div>
  ...
  const DesktopComponent = () => (
    <div className="card workorder-item">
      ...
      <div style={{ width: '20%' }} className="workorder-field">
      ...
      <div className="workorder-field">
        <h2>{props.remark && props.remark}</h2>
      </div>
      <div className="workorder-field">
        {props.time_completed ? (
          <span className="tag is-success">Finished</span>
