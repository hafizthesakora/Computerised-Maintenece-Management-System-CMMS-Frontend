...
export default function WorkOrderForm(props) {
  ...
  const handleSubmit = (event) => {
    ...
    state.technician === 'hafiz.toyyib@external.eni.com'
      ? (state.email = 'hafiz.toyyib@external.eni.com')
      : null;
    // eslint-disable-next-line no-unused-expressions
    state.technician === 'Technician Two'
      ? (state.email = 'hafiz.toyyib@external.eni.com')
      : null;
    ...
    axios
      .post('/workorder', workorder, {
        headers: { 'Content-Type': 'application/json' },
      })
      .then((res) => {
        axios.get('/workorder').then((res) => {
      ...
      {!submit && (
        ...
        >
          <div className="form-content">
            ...
            <div className="field">
              ...
              <div className="select">
                <select
                  ...
                  onChange={(event) =>
                    changeState('technician', event.target.value)
                  }
                >
                  <option key="select" disabled value="">
                    Select Technician
                  </option>
                  <option key="hafiz.toyyib@external.eni.com">
                    hafiz.toyyib@external.eni.com
                  </option>
            ...
            <button className="button is-link submit" type="submit">
              Submit
            </button>
          </div>
          <div className="form-image">
            <img
              src="https://image.freepik.com/free-vector/illustration-characters-fixing-cogwheel_53876-40796.jpg"
              alt="form"
            />
          </div>
        </form>
