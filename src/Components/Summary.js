...
export default function Summary(props) {
  ...
  const printSummary = () => {
    htmlToImage
      .toPng(document.getElementById('summaryPage'), { quality: 0.95 })
  ...
  */
  ...
  const workorderFilteredByMonth = function (workorders, year) {
    ...
    for (const workorder of workorders) {
      ...
      if (workorder.created_on.split('T')[0].split('-')[0] === year) {
        output[
          workorder.created_on.split('T')[0].split('-')[1] - 1
        ].total_workorder_created += 1;

        if (
          !workorder.time_completed ||
          workorder.time_completed.split('T')[0].split('-')[1] >
            workorder.created_on.split('T')[0].split('-')[1]
        ) {
          output[
            workorder.created_on.split('T')[0].split('-')[1] - 1
          ].unfinished_workorder += 1;
        }

        if (workorder.time_completed) {
          output[
            workorder.time_completed.split('T')[0].split('-')[1] - 1
          ].completed_workorder += 1;
        }
  ...
  let workOrderSummary2021 = workorderFilteredByMonth(workorder, '');
  const [workOrderSummary, setWorkOrderSummary] =
    useState(workOrderSummary2021);
  ...
  const calculateMostEfficientMonth = function (workorders) {
    ...
    return `The most efficient month throughout the year is ${mostEfficientMonth},
              The average time to complete the workorders in this month is ${(
                leastAvgDuration /
                1000 /
                60
              ).toFixed(2)} mins`;
  ...
  const workOrderFilterByYear = function (workorder, year) {
    return workorder.filter(
      (workorder) => workorder.created_on.split('T')[0].split('-')[0] === year
    );
  };
  const calculateEachAvgDuration = function (workorders, technician) {
    let total_duration = 0;
    for (const workorder of workorders) {
      if (workorder.duration && workorder.technician === technician)
  ...
  const dataForEachTech = technicianArray.map((technician) => {
    if (technician !== null) {
      return {
        name: technician,
        completed_tasks: completedWorkOrderByTechnician(
          workOrderFilterByYear(workorder, year),
          technician
        ).length,
        avg_duration:
          calculateEachAvgDuration(
            workOrderFilterByYear(workorder, year),
            technician
          ) /
          1000 /
  ...
  const getMostEfficientTechnician = function (technicianData) {
    ...
    return `The most efficient technician is ${mostEfficientTech}, ${mostEfficientTech} completes one work order in an average of ${lowest_avg_duration.toFixed(
      2
    )} hours`;
  };
  ...
  const mostHardWorkingTechnician =
    getMostHardWorkingTechnician(dataForEachTech);
  ...
  return user && user.email === 'hafizthesakora@gmail.com' ? (
    <>
      <div className="summary-page">
        <h1 className="title summary-header">
          Summary of <Dropdown />
          <button
        ...
        <div className="summary-content" id="summaryPage">
          ...
          <div className="card summary-field">
            <h1>
              <b>Work Order Summary for year {year}</b>
            </h1>
            <ComposedChart
              width={500}
              height={400}
            ...
            >
              ...
              <Area
                type="monotone"
                dataKey="total_workorder_created"
                fill="#8884d8"
                stroke="#8884d8"
              />
          ...
          <div className="card summary-field">
            <h1>
              <b>Work Orders Completed by Technicians for year {year}</b>
            </h1>
          ...
          <div className="card summary-field">
            <h1>
              <b>Avg Time Spent on Workorder for each Technician</b>
            </h1>
            <BarChart
              width={500}
              height={300}
