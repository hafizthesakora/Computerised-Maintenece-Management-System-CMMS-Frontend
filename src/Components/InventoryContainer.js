...
export default function WorkOrderContainer(props) {
  ...
  const handleQuery = (inventory) => {
    ...
    if (state.category && !state.item) {
      query = inventory.props.category
        .toLowerCase()
        .includes(state.category.toLowerCase());
    }
    if (!state.category && state.item) {
      query = inventory.props.item
        .toLowerCase()
        .includes(state.item.toLowerCase());
    }
    if (state.category && state.item) {
      query =
        inventory.props.item.toLowerCase().includes(state.item.toLowerCase()) &&
        inventory.props.category
          .toLowerCase()
          .includes(state.category.toLowerCase());
    }
    return query;
  ...
  const parsedInventoryByQuery =
    validState() &&
    parsedInventory.filter((inventory) => handleQuery(inventory));
