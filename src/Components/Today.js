...
export default function Today(props) {
  ...
  const workOrderByTechnician = function (workorders, technician) {
    return workorders.filter(
      (workorder) => workorder.technician === technician
    );
  };
  ...
  const summaryForEachTechnicain = technicianArray.map((technician) => (
    <p key={technician}>
      {technician} has {workOrderByTechnician(props.today, technician).length}{' '}
      unfinished tasks
    </p>
  ));
  ...
  return user && user.email === 'hafizthesakora@gmail.com' ? (
    <>
      <div className="banner">
        <div className="today-panel">
          ...
          <div className="card today-summary">
            <h2 className="card-header card-header-title">
              There are {props.today.length} unfinished tasks
            </h2>
            <div>
              <div className="content today-summary-content">
                <div className="tech-summary">{summaryForEachTechnicain}</div>
                <PieChart title="hello" width={300} height={200}>
                  <Pie
                    ...
                    fill="#fff"
                  ></Pie>
                  <Legend wrapperStyle={{ top: 150, left: -25 }} />
                </PieChart>
              </div>
            </div>
          </div>
      ...
      <div className="workorder-container">{parsedWorkOrders}</div>
    </>
  ) : (
    <></>
  );
