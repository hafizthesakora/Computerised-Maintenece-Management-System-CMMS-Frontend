...
export default function WorkOrderContainer(props) {
  ...
  const handleQuery = (workOrder) => {
    ...
    if (state.title && !state.tech_name && !state.from_date && !state.to_date) {
      query = workOrder.props.title
        .toLowerCase()
        .includes(state.title.toLowerCase());
    }
    if (!state.title && state.tech_name && !state.from_date && !state.to_date) {
      query = workOrder.props.technician
        .toLowerCase()
        .includes(state.tech_name.toLowerCase());
    }
    if (state.title && state.tech_name && !state.from_date && !state.to_date) {
      query =
        workOrder.props.technician
          .toLowerCase()
          .includes(state.tech_name.toLowerCase()) &&
    ...
    if (!state.title && !state.tech_name && state.from_date && state.to_date) {
      query =
        workOrder.props.created_on <= state.to_date &&
        workOrder.props.created_on >= state.from_date;
    }
    if (state.title && state.tech_name && state.from_date && state.to_date) {
      query =
        workOrder.props.created_on <= state.to_date &&
  ...
  const parsedWorkOrders =
    props.workorder &&
    props.workorder.map((workOrder) => {
      return <WorkOrderItems {...workOrder} key={workOrder._id} />;
    });

  const parsedWorkOrdersByQuery =
    validState() &&
    parsedWorkOrders.filter((workOrder) => handleQuery(workOrder));

  const parsedWorkOrdersByName =
    user &&
    parsedWorkOrders.filter(
      (workOrder) => workOrder.props.technician === user.name
    );

  const sortedWorkOrderByStatus =
    parsedWorkOrdersByName &&
    parsedWorkOrdersByName.sort((a, b) =>
          ...
          <div className="filters">
            ...
            <div className="filter-input">
              <input
                ...
                onChange={(event) =>
                  changeState('tech_name', event.target.value)
                }
              />
            </div>
            <div className="filter-input">
  ...
  const technicianView = (
    <>
      ...
      <div className="workorder-container">{sortedWorkOrderByStatus}</div>
    </>
  );
